from global_group_poly_pricing import GlobalGroupPolyClusterExplanationPP01
from global_group_poly_master import GlobalGroupPolyClusterExplanationSolver
import pandas as pd
import math
import numpy as np
...
class GlobalGroupPolyClusterExplainCG(object):
    '''
    Object to create a cluster explanation using column gen
    '''
    
    def __init__(self, X, clusters, weights, W, b, bad_hps = 0, alpha = 0, alpha_type = 'global', M = 1, beta = 1, objective='num_hp', colGen_allowed = True, new_hp_prefix = 'CG_'):
        ...
        self.master = GlobalGroupPolyClusterExplanationSolver(self.X, self.clusters, self.weights, self.W, self.b, self.alpha, self.alpha_type,  self.bad_hps, objective = objective)
        
        if self.colGen_allowed:
            self.pricing = {}
            for cl in np.unique(self.clusters):
                self.pricing[cl] = GlobalGroupPolyClusterExplanationPP01(self.X, self.clusters,  cl, M = M, beta = beta, objective = objective)
                
        
    def solve(self, verbose = False, colGen = True, timeLimit = 300):
        
        if not self.colGen_allowed:
            print('Col gen not allowed on this object.')
            colGen = False
        ...
        while colGen:
           ...
           #print('Objective Value of Pricing:', pricing_res['objVal'])
            ...
            if new_hp_flag:
                ...
                self.master = GlobalGroupPolyClusterExplanationSolver(self.X, self.clusters, self.weights, self.W, self.b, self.alpha, self.alpha_type, self.bad_hps, objective = self.objective)
