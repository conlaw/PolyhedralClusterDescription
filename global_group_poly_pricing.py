...
class GlobalGroupPolyClusterExplanationPP01(object):
    ...
    def init_dv(self):
        #Create w, b vars
        self.w_pos = self.m.addMVar(int(self.X.shape[1]/2), lb = 0, ub = self.M, vtype = gp.GRB.INTEGER, name='w_pos')
        self.w_neg = self.m.addMVar(int(self.X.shape[1]/2), lb = 0, ub = self.M, vtype = gp.GRB.INTEGER, name='w_neg')
        ...
        #create y vars
        self.y_pos = self.m.addMVar(int(self.X.shape[1]/2), vtype = gp.GRB.BINARY, name='y_pos')
        self.y_neg = self.m.addMVar(int(self.X.shape[1]/2), vtype = gp.GRB.BINARY, name='y_neg')
    ...
    def add_constr(self):
        ...
        self.y_nondegen = self.m.addConstr(self.y_pos + self.y_neg <= np.ones(int(self.X.shape[1]/2)), name='y_nondegen')
        ...
        br = int(X_cl.shape[1]/2)
        #df.iloc[:,(int(df.shape[1]/2)-1):].shape
        
        self.m.addConstr(X_sub[:,br:]@ self.w_pos -  X_sub[:,:br] @ self.w_neg + np.ones((X_sub.shape[0],1))@self.b   >= self.epsilon-bigM+bigM*self.delta_i[self.clusters != self.k] , name='hp_sub')
        self.m.addConstr(X_cl[:,:br] @ self.w_pos - X_cl[:,br:] @ self.w_neg + np.ones((X_cl.shape[0],1))@self.b  <= self.epsilon+bigM*self.delta_i[self.clusters == self.k] , name='hp_cl')


    def set_obj(self):
        #Minimize # of hps used
        #print(self.mu_self, self.mu_prime)
        self.m.setObjective( - self.mu_self @ self.delta_i[self.clusters == self.k] + self.mu_prime @ self.delta_i[self.clusters != self.k] + self.gamma @ self.y_pos + self.gamma @ self.y_neg , gp.GRB.MAXIMIZE)
    ...
    def getAllHPs(self):
        ...
        for i in range(solCount):
            ...
            if self.pricing_const - obj >= 0:
                break
            W[i] = np.array(self.m.getAttr(GRB.Attr.Xn)[0:int(self.X.shape[1]/2)]) -np.array(self.m.getAttr(GRB.Attr.Xn)[int(self.X.shape[1]/2):self.X.shape[1]])
            B[i] = np.array([self.m.getAttr(GRB.Attr.Xn)[self.X.shape[1]]])
